...
name = 'Lena';
dim = 32;
img_rgb = imread(['img_filtro\', name, '.jpeg']);
...
[status1, message1, messageId1] = copyfile([filepath_behav, 'R', filename, '.txt'], [filepath_impl, 'R', filename, '.txt']);
[status2, message2, messageId2] = copyfile([filepath_behav, 'R', filename, '.txt'], [filepath_impl, 'R', filename, '.txt']);
[status3, message3, messageId3] = copyfile([filepath_behav, 'R', filename, '.txt'], [filepath_impl, 'R', filename, '.txt']);

% Check if the copy was successful
if status1
    disp('File copied successfully.');
else
    disp(['File copy failed. Error: ', message1]);
end
